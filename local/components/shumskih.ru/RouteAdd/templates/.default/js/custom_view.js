ymaps.modules.define('MultiRouteCustomView', [
    ...
    defineClass(CustomView, {
        ...
        processSuccessRequest: function (multiRouteModel, e) {
            ...
            if (routes.length) {
                ...
                for (let i = 0, l = routes.length; i < l; i++) {
                    print_route += '<div class="box-route">';
                    print_route += this.processRoute(i, routes[i]);
                    print_route += '</div>';
                }

                result.push('<div class="box-routes">' +
                    '<h4>Всего маршрутов: ' + routes.length + '</h4>' +
                    print_route +
                    '</div>');
            } else {
        ...
        processRoute: function (index, route) {
            ...
            return '<b>' + (index + 1) + '</b>' + ". " + this[processorName](route);
        },

        processDrivingRoute: function (route) {
            var result = ["<b>Автомобильный маршрут.<b/>"];
            result.push(this.createCommonRouteOutput(route));
            return result.join("<br/>");
        },

        processMasstransitRoute: function (route) {
            var result = ["<b>Маршрут на общественном транспорте.</b>"];
            result.push(this.createCommonRouteOutput(route));
        ...
        processPedestrianRoute: function (route) {
            var result = ["<b>Пешеходный маршрут.</b>"];
            result.push(this.createCommonRouteOutput(route));
            return result.join("<br/>");
        },
